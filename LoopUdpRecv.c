...
int main(int argc, char *argv[]){
    ...
    //sendto
    strcpy(buf,"request:"+echoClientPort);
    size = sizeof("request:"+echoClientPort);
    if(sendto(sock,"request:"+echoClientPort,size,0,(struct sockaddr*)&server_addr,sizeof(server_addr)) < 0){
        perror("sendto");
        return 1;
    }
